...
/**
 * Robustly processes HTML content to extract text and a primary image URL.
 * This approach is safer than modifying HTML strings directly.
 */
const processHTMLContent = (html) => {
  ...
  // Remove script and style tags to avoid execution and style conflicts
  doc.querySelectorAll("script, style").forEach((el) => el.remove());

  // Get the first image src, if it exists
  ...
  // Return all parts for flexible rendering
  return {
    text,
    image: imageSrc,
    htmlContent: doc.body.innerHTML, // Return the sanitized HTML
  };
};
...
const normalizeQuestion = (raw, idx, sections) => {
  ...
  // Process the main question body
  const {
    text: questionText,
    image: questionImage,
  ...
  // Process options, preserving original index
  const processedOptions = (raw.options ?? raw.choices ?? raw.opts ?? []).map(
    (optionHtml, index) => {
      ...
      return {
        text: optionText,
        image: optionImage,
        html: optionFullHtml, // Full sanitized HTML for rendering
        originalIndex: index,
  ...
  return {
    ...
    id,
    qid: raw.qid ?? id,
    questionText, // Just the text part of the question
    questionImage, // The first image URL from the question
    questionHtml, // Full sanitized HTML of the question
    processedOptions, // The array of processed option objects
    marks,
    kind,
    section: sec,
    __idx: idx,
...
const parseSections = (sectionsString, totalQuestions = 0) => {
  ...
  try {
    ...
    return [{ name: " ", start: 1, end: totalQuestions || 0 }];
  }
};
  ...
  ({
    ...
    return (
      <Box key={question.id} sx={{ mb: 5 }}>
        ...
        <Box>
          <Box sx={{ lineHeight: 1.8, typography: "body1" }}>
                  ...
                  <FormControlLabel
                    ...
                    label={
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        {opt.text && <Typography>{opt.text}</Typography>}
                        {opt.image && (
                          <img
...
const Page3 = () => {
  ...
  // In page3.jsx, replace the entire fetchAllData function
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      // First, always fetch the latest question data from the API
      ...
      const fetchedQuestionsData = Array.isArray(questionsRes.data) ? questionsRes.data : [];
      const testPaperData = testPaperRes.data || {};
      
      const parsedSections = parseSections(testPaperData.sections, fetchedQuestionsData.length);
      const normalized = fetchedQuestionsData.map((q, idx) => normalizeQuestion(q, idx, parsedSections));
      
      // Load duration and sections immediately
      setSections(parsedSections);
      if (testPaperData.duration) {
        setTimeLeft(num(testPaperData.duration, 0) * 60);
      }
      
      // Set questions state once
      setQuestions(normalized);

      // Now, check localStorage to see if we should restore a session
      const cachedPaperId = localStorage.getItem("currentPaperId");
      if (String(cachedPaperId) === String(paperId)) {
        console.log("Resuming same paper. Attempting to load state from localStorage.");
        ...
        if (cachedAns && cachedStatus) {
          ...
          return; // IMPORTANT: Exit here after successfully loading state
        }
      }

      // If we are here, it means it's a new test or there's no saved state.
      // So, we create a fresh state.
      ...
      setAnswers({}); // Clear any previous test's answers
      setQuestionStatus(initialStatus);

    } catch (e) {
      setError(e.response?.data?.message || e.message || "Failed to fetch test data.");
    } finally {
      setLoading(false);
    }
  }, [paperId, authState]);
  ...
  const navigateToQuestion = useCallback(
    async (index) => {
      ...
      try {
        ...
        console.error("Navigation halted because save failed.", e);
      }
    },
    [
      answers,
  ...
  const saveAndNext = useCallback(async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        ...
        console.error("Save failed on last question.", e);
      }
    }
  }, [
    navigateToQuestion,
  ...
  const checkLocalStorage = useCallback(() => {
    ...
    console.log(
      "Saved answers in localStorage:",
      savedAnswers ? JSON.parse(savedAnswers) : null
    );
    ...
    <ErrorBoundary>
      <ThemeProvider theme={theme}>
        <StyledContainer>
          ...
          // In page3.js, find and replace the BottomNav component with this:
...
<BottomNav elevation={3}>
  ...
  >
              ...
              <Button
                ...
                disabled={
                  !(answers[currentQuestion?.id] !== undefined && answers[currentQuestion?.id] !== null && (
                    ...
                    (typeof answers[currentQuestion?.id] === 'string' && String(answers[currentQuestion?.id]).trim() !== '') ||
    ...
    {/* This is the new popup menu for mobile view */}
    <Box
      sx={{
        position: "relative",
