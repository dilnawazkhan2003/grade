  ...
  useEffect(() => {
    const fetchTestPapers = async () => {
      ...
      try {
        ...
        // --- LOGIC TO CHECK ATTEMPTED STATUS ---
        // 1. Get the list of submitted IDs from localStorage.
        const submittedPapersRaw = localStorage.getItem('submittedPapers');
        const submittedIds = submittedPapersRaw ? JSON.parse(submittedPapersRaw) : [];
        const submittedIdsSet = new Set(submittedIds); // Use a Set for faster checking

        // 2. Add a 'status' field to each paper from the API response.
        const papersWithStatus = response.data.map(paper => ({
          ...paper,
          status: submittedIdsSet.has(String(paper.id)) ? 'submitted' : 'not_attempted'
        }));
        
        // 3. Set the state with the new data containing the status.
        setTestPapers(papersWithStatus);
        // --- END OF NEW LOGIC ---
                      ...
                      <Box sx={{ 
                        ...
                        {isSubmitted ? (
                          <Button 
                            ...
                            onClick={() => navigate(`/result/${paper.id}`)}
                          >
                            View Results
                          </Button>
                        ) : (
                          <Button 
                            ...
                            color="primary" 
                            size="large"
                            onClick={() => navigate(`/page2/${paper.id}`)}
                          >
                            Start Test
                          </Button>
                        )}
